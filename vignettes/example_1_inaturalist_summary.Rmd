...
This end-to-end example shows how to use **recorderFeedback** with iNaturalist data.
It is written for new users, so each stage is explicit: initialise project, load recipients, fetch observations, render content, then distribute.
...
Run subsequent commands from that project directory so relative paths in `config.yml` are valid.
...
(To keep this vignette self-contained, the template text is printed directly.)
...
Finally, send reports by email using configured SMTP settings.
For first tests, keep `test_mode: TRUE` in `config.yml`.
...
- No records returned: verify the iNaturalist `recipient_id` values are valid user IDs.
- API errors: rerun later or reduce request size while testing.
- Render failure: test with `rf_render_single()` and inspect the template chunk that fails.
