...
This file gives an AI coding agent the context needed to help a user run
the `recorderFeedback` package end to end.

## Purpose

Use this repository to generate personalized feedback content for
recipients, then optionally dispatch via SMTP email.

## Project Type

- R package with workflow helpers and templates.
- Typical user flow is project initialization, data loading, rendering,
  and dispatch.
...
- [`rf_get_recipients()`](https://simonrolph.github.io/recorderFeedback/reference/rf_get_recipients.md):
  run the configured script to update recipients CSV.
- [`rf_get_data()`](https://simonrolph.github.io/recorderFeedback/reference/rf_get_data.md):
  run the configured script to update records CSV.
- `rf_verify_data(verbose = TRUE)`: verify required columns and data
  readiness.
- `rf_render_single(recipient_id = 1)`: render one recipient for
  debugging.
- `rf_render_all(batch_id)`: run the targets pipeline for a full batch.
- `rf_verify_batch(batch_id, verbose = TRUE)`: validate rendered outputs
  and metadata.
...
If any configured file is missing, agent should stop and report the
missing path with a clear fix.

## Selective Rendering Contract

`rf_render_all(batch_id)` supports selection via
`recipient_select_script`. That script must define:
...
- Data frame with `recipient_id`, `selected`, and optional
  `skip_reason`.

Expected behavior:

- Selected recipients are rendered.
- Non-selected recipients appear in `renders/<batch_id>/meta_table.csv`
  with `render_status = "skipped"` and no HTML `file`.
...
8.  Optional dispatch:
    - if SMTP configured and user confirms, run
...
- If a single recipient fails in batch: reproduce with
  [`rf_render_single()`](https://simonrolph.github.io/recorderFeedback/reference/rf_render_single.md).
- If emails are skipped unexpectedly: inspect `recipient_select_script`
  and `render_status` values in meta table.
