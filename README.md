...
[![](https://img.shields.io/badge/devel%20version-0.9.0-blue.svg)](https://github.com/BiologicalRecordsCentre/recorderFeedback)
[![R build
status](https://github.com/BiologicalRecordsCentre/recorderFeedback/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/BiologicalRecordsCentre/recorderFeedback/actions)
...
- `rf_get_recipients()` and `rf_get_data()` populate your `data/`
  inputs.
...
- `rf_dispatch_smtp()` sends emails with dry-run, resume, retries, and
  rate limiting.
...
- `config.yml`: central settings (file paths, templates, email options,
  test mode)
...
Then open `my_feedback_project/config.yml` and update paths/settings for
your use case.
...
- `templates/content.Rmd` for the feedback content recipients will
  receive
...
- Template: the `Rmd` file that turns data into recipient-specific
  content.
...
2.  Run `rf_dispatch_smtp(batch_id, dry_run = TRUE)` to validate rows
    and files.
...
- `mail_body_prefix_template` / `mail_body_suffix_template` inject
  optional HTML around each rendered email using the same template
  variables.
- `mail_attachments_col` / `mail_inline_images_col` define optional
  `meta_table.csv` columns with per-recipient file paths for attachments
  and inline media.
- `preview_only` with `preview_n` or `preview_recipient_ids` writes
  sampled previews to `renders/<batch_id>/dispatch_preview/`.
...
- optional recorder schema checks for columns like `date`, `species`,
  and `site`
